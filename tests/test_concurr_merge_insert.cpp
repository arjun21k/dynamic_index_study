...
void search_disk_index(const std::string              &index_prefix_path,
  ...
  int res = _pFlashIndex->load(index_prefix_path.c_str(), num_threads, true);
  if (res != 0) {
...
void search_kernel(diskann::MergeInsert<T>        &merge_insert,
  ...
  if (getrlimit(RLIMIT_STACK, &limit) == 0) {
    std::cout << "Stack Size Limit: " << limit.rlim_cur / (1024 * 1024)
              << " MB\n";
  } else {
    std::cerr << "Failed to get stack limit\n";
  }
  struct mallinfo2 mi = mallinfo2();
  std::cout << "Total Heap Allocated: " << mi.arena / (1024 * 1024) << " MB\n";
  std::cout << "Total Free Chunks: " << mi.fordblks / (1024 * 1024) << "
  MB\n";*/
  ...
  //     "/localscratch/arjun/freshdiskann/sift10k/siftsmall/sift_query.fbin";
  ...
  if (print_stats) {
    ...
    std::cout << std::setw(4) << "Ls" << std::setw(12) << "QPS "
              ...
              << "Mean disk IOs" << std::setw(12) << "IO time(us)"
              << std::setw(10) << "CPU (us)" << std::endl;
  ...
  // std::cout << "query_result_tags size = " << query_result_tags.size()
  //         << std::endl;
...
#pragma omp parallel for num_threads(NUM_SEARCH_THREADS)
    ...
    std::cout << std::setw(4) << L << std::setw(12) << qps << std::setw(18)
              ...
              << std::setw(12) << mean_io_us << std::setw(10) << mean_cpuus
              << std::endl;
    delete[] stats;
  }
  diskann::aligned_free(query);
...
void run_all_iters(std::string base_prefix, std::string merge_prefix,
  ...
  std::cout << "npts = " << npts << ", Dim passed to Merge_insert() = " << ndims
            << std::endl;
