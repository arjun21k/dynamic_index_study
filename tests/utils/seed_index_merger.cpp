...
void run(const uint32_t base_count, const uint32_t num_mem_indices,
  ...
  while (count++ < num_cycles) {
    ...
    float active_tags_sampling_rate = (float) ((
        std::min) ((1.0 * delete_count) / (1.0 * ((double) active_tags.size())),
                   1.0));
    ...
    float inactive_tags_sampling_rate = (float) ((
        std::min) ((1.0 * incr_count) / (1.0 * ((double) inactive_tags.size())),
                   1.0));
