...
void sanityCheck(const char* origfile, const char* binfile,
  ...
  diskann::cout << "Checking first " << vectorsToCheck
  ...
  if (mismatchCount == 0) {
    ...
    diskann::cout << "Conversion failed!!! Found " << mismatchCount
  ...
  while (std::getline(fin, line)) {
    ...
    if (vector.size() != 0) {
      ...
      if (allVecs.size() % 10000 == 0) {
        diskann::cout << "Processed " << allVecs.size() << " records."
                      << std::endl;
      }
...
int main(int argc, char** argv) {
  ...
  diskann::cout << "Converting (" << argv[1] << ") axon file: " << argv[2]
