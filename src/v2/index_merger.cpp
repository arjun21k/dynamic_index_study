...
namespace diskann {
  ...
  StreamingMerger<T, TagT>::StreamingMerger(
    ...
    // book keeping
    std::cout << "StreamingMerger constructor start: " << this << std::endl;
    this->ndims = ndims;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_INSERT_THREADS)
  ...
  void StreamingMerger<T, TagT>::compute_deleted_ids() {
    /*diskann::cout << "Indise compute_deleted_ids(), this->disk_deleted_ids.size() = " << this->disk_deleted_ids.size()
	    << ", this->deleted_tags.size() = " << this->deleted_tags.size() << ", this->u_deleted_tags.size() = "
	    << this->u_deleted_tags.size() << "\n";
    int u_count = 0;*/
    ...
    //diskann::cout << "Found " << u_count << " tags to delete from SSD-DiskANN\n";
...
#pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
  ...
  void StreamingMerger<T, TagT>::populate_deleted_nhoods() {
    ...
    diskann::cout << "Inside populate_deleted_nhoods, this->disk_deleted_ids.size() = " << this->disk_deleted_ids.size() 
	    << " , this->max_node_len = " << this->max_node_len << " , ROUND_UP(this->max_node_len, 32) = " << 
    ...
    diskann::cout << "backing_buf_size = " << backing_buf_size << "\n";
  ...
  void StreamingMerger<T, TagT>::write_tag_file(
    ...
    alloc_aligned(((void **) &cur_tags), allocSize, sizeof(TagT));
...
#pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
  ...
  void StreamingMerger<T, TagT>::merge(const char *                    disk_in,
    ...
    this->final_index_file =
        this->_single_file_index ? this->disk_index_out_path + "_disk.index"
	...
	//this->u_deleted_tags.insert((*deleted_tags_vector)[i]);
      }
    }
  ...
  void StreamingMerger<T, TagT>::mergeImpl() {
    ...
    diskann::cout << "Patch Phase: Start\n";
