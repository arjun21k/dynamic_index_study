...
namespace diskann {
  ...
  size_t calculate_num_pq_chunks(double final_index_ram_limit,
    ...
    size_t num_pq_chunks = (size_t) (std::floor) (_u64(final_index_ram_limit /
                                                       (double) points_num));
  ...
  int build_merged_vamana_index(
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      if (tags_enabled) {
        ...
        std::vector<uint32_t> tags(base_num);
        std::iota(tags.begin(), tags.end(), 0);
        _pvamanaIndex->build(base_file.c_str(), base_num, paras, tags);
      }
