...
#define NODE_SECTOR_NO(node_id)                      \
  (1 + (nnodes_per_sector > 0                        \
            ? ((_u64) (node_id)) / nnodes_per_sector \
            : node_id * DIV_ROUND_UP(max_node_len, SECTOR_LEN)))
...
#pragma omp critical
  ...
  std::vector<bool> PQFlashIndex<T, LabelT>::read_nodes(
    ...
    alloc_aligned((void **) &buf, node_ids.size() * num_sectors * SECTOR_LEN,
                  SECTOR_LEN);
    ...
    for (size_t i = 0; i < node_ids.size(); ++i) {
      ...
      AlignedRead read;
      read.len = num_sectors * SECTOR_LEN;
      read.buf = buf + i * num_sectors * SECTOR_LEN;
      read.offset = NODE_SECTOR_NO(node_id) * SECTOR_LEN;
    ...
    defined(USE_BING_INFRA)  // this block is to handle failed reads in
      ...
      if (coord_buffers[i] != nullptr) {
        ...
        memcpy(coord_buffers[i], node_coords, data_dim * sizeof(T));
      }
  ...
  void PQFlashIndex<T, TagT>::load_cache_list(
    ...
    for (_u64 block = 0; block < num_blocks; block++) {
      ...
      std::vector<_u32>                        nodes_to_read;
      std::vector<T *>                         coord_buffers;
      std::vector<std::pair<_u32, unsigned *>> nbr_buffers;
      for (_u64 node_idx = start_idx; node_idx < end_idx; node_idx++) {
        ...
        coord_buffers.push_back(coord_cache_buf + node_idx * aligned_dim);
        nbr_buffers.emplace_back(0,
                                 nhood_cache_buf + node_idx * (max_degree + 1));
      }

      // reader->read(read_reqs, ctx);
      //  issue the reads
      auto read_status =
          read_nodes(nodes_to_read, coord_buffers, nbr_buffers, ctx);
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void PQFlashIndex<T, TagT>::cache_bfs_levels(
    ...
    while ((node_list.size() + cur_level->size() < num_nodes_to_cache) &&
      ...
      for (size_t block = 0; block < nblocks && !finish_flag; block++) {
        ...
        std::vector<std::pair<_u32, _u32 *>> nbr_buffers;
        ...
        for (size_t cur_pt = start; cur_pt < end; cur_pt++) {
          ...
          nbr_buffers.emplace_back(0, new _u32[max_degree + 1]);
        }
        ...
        for (uint32_t i = 0; i < read_status.size(); i++) {
          if (read_status[i] == false) {
            ...
            _u32  nnbrs = nbr_buffers[i].first;
            _u32 *nbrs = nbr_buffers[i].second;

            // explore next level
            for (_u32 j = 0; j < nnbrs && !finish_flag; j++) {
  ...
  void PQFlashIndex<T, TagT>::use_medoids_data_as_centroids() {
    ...
    std::vector<std::pair<_u32, _u32 *>> nbr_bufs;

    for (uint64_t cur_m = 0; cur_m < num_medoids; cur_m++) {
      nodes_to_read.push_back(medoids[cur_m]);
      medoid_bufs.push_back(new T[data_dim]);
      nbr_bufs.emplace_back(0, nullptr);
    }

    auto read_status = read_nodes(nodes_to_read, medoid_bufs, nbr_bufs, ctx);

    for (uint64_t cur_m = 0; cur_m < num_medoids; cur_m++) {
      if (read_status[cur_m] == true) {
        for (uint32_t i = 0; i < data_dim; i++)
          centroid_data[cur_m * aligned_dim + i] = medoid_bufs[cur_m][i];
      } else {
        throw ANNException("Unable to read a medoid", -1, __FUNCSIG__, __FILE__,
                           __LINE__);
      }
  ...
  int PQFlashIndex<T, TagT>::load(const char *index_prefix, _u32 num_threads,
    ...
    std::cout << "Inside PQFlashIndex" << std::endl;
    std::cout << "Num threads = " << num_threads << std::endl;
    std::cout << "Index prefix = " << index_prefix << std::endl;
    std::cout << "new_index_format = " << new_index_format << std::endl;
    std::cout << "single_index_file = " << this->single_index_file << std::endl;
    ...
    if (false == this->single_index_file) {
      ...
      disk_index_file = disk_index_file + "_disk.index";
      this->_disk_index_file = disk_index_file;
    }

    std::cout << "disk_index_file = " << disk_index_file << std::endl;
#ifdef EXEC_ENV_OLS
    ...
    std::cout << "EXEC_ENV_OLS" << std::cout;
...
#else
    diskann::cout << "pq_compressed_vectors path = " << pq_compressed_vectors
                  << std::endl;
...
// read index metadata
    ...
    if (this->enable_tags) {
      ...
      if (!this->single_index_file) {
        ...
        diskann::cout << "from " << disk_index_file
                      << " and offset = " << tags_offset << std::endl;
  ...
  size_t PQFlashIndex<T, TagT>::cached_beam_search(
    ...
    for (uint32_t i = 0; i < l_search && res_count < k_search; i++) {
      ...
      /*if (res_tags != nullptr && this->tags != nullptr) {  // &&
        // expanded_nodes_info[i].id < 84284) {
        res_tags[res_count] = this->tags[expanded_nodes_info[i].id];
      }*/
      if (res_tags != nullptr) {
      ...
      if (!frontier.empty()) {
        ...
        for (_u64 i = 0; i < frontier.size(); i++) {
          ...
          fnhood.second = sector_scratch + num_sectors_per_node *
                                               sector_scratch_idx * SECTOR_LEN;
          sector_scratch_idx++;
          frontier_nhoods.push_back(fnhood);
          frontier_read_reqs.emplace_back(
              NODE_SECTOR_NO(((size_t) id)) * SECTOR_LEN,
              num_sectors_per_node * SECTOR_LEN, fnhood.second);
          if (stats != nullptr) {
            stats->n_4k++;
            stats->n_ios++;
          }
            ...
            /*            if ((!(node.nnbrs > 0)) ||
               ...
               *(OFFSET_TO_NODE_COORDS(buf_start) + i)
               ...
               i)
                        ...
                        */
            /*if (node.nnbrs <= 0) {
              std::cout << "deleted_nodes.size = " << deleted_nodes.size()
                        << " , node.nnbrs = " << node.nnbrs
                        << " , n_scanned = " << n_scanned << std::endl;
            }*/
