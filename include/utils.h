...
namespace diskann {
  ...
  inline void load_bin_impl(std::basic_istream<char>& reader, T*& data,
    ...
    data = new T[npts * dim];
    // std::cout << "load_bin_impl npts = " << npts << " dim = " << dim
    //         << std::endl;
    // std::cout << "npts * dim = " << npts * dim << std::endl;
    reader.read((char*) data, npts * dim * sizeof(T));

    /*std::streamsize bytesRead = reader.gcount();  // Get actual bytes read
    std::cout << "Bytes read: " << bytesRead << std::endl;

    if (bytesRead < npts * dim * sizeof(T)) {
      std::cerr << "Warning: Incomplete read!" << std::endl;
    }*/
  }
  ...
  template<typename T>
  __attribute__((no_sanitize_address)) void load_aligned_bin(
      const std::string& bin_file, T*& data, size_t& npts, size_t& dim,
      size_t& rounded_dim, size_t offset = 0) {
    diskann::cout << "Reading bin file " << bin_file << " at offset " << offset
                  << "..." << std::flush;
    std::ifstream reader(bin_file, std::ios::binary | std::ios::ate);
