...
namespace diskann {
  ...
  class PQFlashIndex {
   public:
    ...
    // coord_buffers: pointers to pre-allocated buffers that coords need to
    // copied to. If null, dont copy. nbr_buffers: pre-allocated buffers to copy
    // neighbors into
    //
    // returns a vector of bool one for each node_id: true if read is success,
    // else false
    //
    DISKANN_DLLEXPORT std::vector<bool> read_nodes(
    ...
    uint64_t nnodes_per_sector =
        0;  // 0 for multi-sector nodes, >0 for multi-node sectors
